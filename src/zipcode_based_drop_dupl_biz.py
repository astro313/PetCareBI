...
def remove_dupl_biz(cities, out_path, columns, catnum=5):
    ...
    for zipc in cities:
        ...
        fff = glob.glob('../data/raw/' + zipc + '_pet_*' + '_2020*.csv')
...
if __name__ == '__main__':
    NYC_review_files = glob.glob('../data/raw/' + '1????_pet_*' + '_2020*.csv')
    SF_review_files = glob.glob('../data/raw/' + '9????_pet_*' + '_2020*.csv')

    zipcodes_SF = np.loadtxt('../data/zip_code_SF.txt', delimiter=',')
    cities_csv_SF = list(map(int, zipcodes_SF))

    zipcodes_NYC = np.loadtxt('../data/zip_code.txt', delimiter=',')
